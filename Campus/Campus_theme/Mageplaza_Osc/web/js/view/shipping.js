    ...
    function ($,
        ...
        return Component.extend({
            ...
            validate: function () {
				...
				/***********************************Самовивіз**********************************/
				var current_shipping = $('#checkout-step-shipping_method .table-checkout-shipping-method .col-method input:checked').val();
				
				if(current_shipping == 'flatrate_flatrate') {
					$("[name='city']").val($('#checkout-step-shipping_method .table-checkout-shipping-method .col-method input:checked ~ label > span').text());
					$("div[name='shippingAddress.street.0'] [name='street[0]']").val($('#checkout-step-shipping_method .table-checkout-shipping-method .col-method input:checked ~ label > span').text());
					var event = document.createEvent('HTMLEvents');
					event.initEvent('change', false, false);
					var a = document.querySelector("[name='city']");
					a.dispatchEvent(event);
					var street = document.querySelector("[name='street[0]']");
					street.dispatchEvent(event);
				}/*  else {
					$("[name='city']").val('');
					$("div[name='shippingAddress.street.0'] [name='street[0]']").val('');
				} */
				
				console.log($('#checkout-step-shipping_method .table-checkout-shipping-method .col-method input:checked').val());
			...
			getCustomerInfo: function () {
				return $.mage.__('You entered as ') + customer.customerData.firstname;
			},
			
			getLogoutCustomerUrl: function () {
				return window.BASE_URL + 'customer/account/logout/';
			}
        });
    }
);
