    ...
    function (_, $, Component, quote, updateItemAction, giftMessageItem, url, $t, modal) {
        ...
        return Component.extend({
            ...
            plusQty: function (item, event) {
				...
				var product_multiplicity = parseInt(target.attr('data-multiplicity'));

                if (qtyIncrements.hasOwnProperty(itemId)) {
                    var qtyDelta = qtyIncrements[itemId];

                    qty = (Math.floor(qty / qtyDelta) + product_multiplicity) * qtyDelta;
                } else {
                    qty += product_multiplicity;
                }
            ...
            minusQty: function (item, event) {
                ...
                if (qtyIncrements.hasOwnProperty(itemId)) {
                    ...
                    qty = (Math.ceil(qty / qtyDelta) - product_multiplicity) * qtyDelta;
                } else {
                    qty -= product_multiplicity;
                }
			...
			getPiecesInThePackage: function(itemId) {
				var item = this.getItem(itemId);
                
				/*return item.piece_packing;*/
                return 1;
            },
			
			getProductPriceVal: function(itemId) {
				var item = this.getItem(itemId);
                
				return parseFloat(item.price).toFixed(2);
            },
			
			getProductTotal: function(itemId) {
				var item = this.getItem(itemId);
                
				return parseFloat(item.row_total).toFixed(2);
            },
			
			getProductPriceOne: function(itemId) {
				var item = this.getItem(itemId);
                
				return item.product_price_one;
            },
