...
let floatingScrollProto = {
    ...
    initWidget() {
        let instance = this;
        let widget = instance.widget = jQuery("<div class='fl-scrolls'></div>");
        jQuery("<div></div>").appendTo(widget).css({width: `jQuery{instance.container.scrollWidth}px`});
    ...
    addEventHandlers() {
        ...
        let eventHandlers = instance.eventHandlers = [
            {
                jQueryel: instance.scrollBody || jQuery(window),
                handlers: {
                    // Don't use `jQuery.proxy()` since it makes impossible event unbinding individually per instance
            ...
            {
                jQueryel: jQuery(instance.container),
                handlers: {
                    scroll() {
        ...
        eventHandlers.forEach(({jQueryel, handlers}) => jQueryel.bind(handlers));
    },
    ...
    updateAPI() {
        ...
        if (!scrollBody) {
            widget.css("left", `jQuery{container.getBoundingClientRect().left}px`);
        }
        jQuery("div", widget).width(scrollWidth);
    ...
    destroyAPI() {
        let instance = this;
        instance.eventHandlers.forEach(({jQueryel, handlers}) => jQueryel.unbind(handlers));
        instance.widget.remove();
        instance.eventHandlers = instance.widget = instance.container = instance.scrollBody = null;
    }
};

jQuery.fn.floatingScroll = function (method = "init") {
    if (method === "init") {
        this.each((index, el) => Object.create(floatingScrollProto).init(jQuery(el)));
    } else if (floatingScrollProto.hasOwnProperty(`jQuery{method}API`)) {
        this.trigger(`jQuery{method}.fscroll`);
    }
    return this;
};

jQuery(document).ready(() => {
    jQuery("body [data-fl-scrolls]").floatingScroll();
});